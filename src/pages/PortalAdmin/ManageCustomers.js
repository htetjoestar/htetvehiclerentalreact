...
const ManageCustomers = () => {
  ...
  return (
  <div>
    <h2 className="m-6 text-2xl font-semibold mb-4">Manage Customer Accounts</h2>
...
<div className="overflow-x-auto max-w-full">
    <table className="min-w-full table-auto border-collapse">
      ...
      <th
        ...
        onClick={() => handleSort('cust_first_name')}
      >
        first name{" "}
        <span className="text-gray-400">
          {sortField === 'cust_first_name' ? (sortOrder === 'asc' ? '▲' : '▼') : '⇅'}
        </span>
      </th>
      <th
        className="border px-4 py-2 text-left cursor-pointer"
        onClick={() => handleSort('cust_last_name')}
      >
        last name{" "}
        <span className="text-gray-400">
          {sortField === 'cust_last_name' ? (sortOrder === 'asc' ? '▲' : '▼') : '⇅'}
        </span>
      </th>
      ...
      <th
        className="border px-4 py-2 text-left cursor-pointer"
        onClick={() => handleSort('cust_created_date')}
      >
        created date{" "}
        <span className="text-gray-400">
          {sortField === 'cust_created_date' ? (sortOrder === 'asc' ? '▲' : '▼') : '⇅'}
        </span>
      </th>
            <th className="border px-4 py-2 text-left">last action</th>      
      <th
        className="border px-4 py-2 text-left cursor-pointer"
        onClick={() => handleSort('cust_modified_date')}
      >
        modified date{" "}
        <span className="text-gray-400">
          {sortField === 'cust_modified_date' ? (sortOrder === 'asc' ? '▲' : '▼') : '⇅'}
        </span>
      </th>
      ...
      <tbody>
        {reservations.map(maint => (
          <tr key={maint.customer_id} style={{ borderBottom: "1px solid #ccc" }}>
            <td>{maint.customer_id}</td>
            <td>{maint.cust_email}</td>
            <td>{maint.cust_first_name}</td>
            <td>{maint.cust_last_name}</td>
            <td>{formatLocalDate(maint.cust_date_of_birth)}</td>
            <td>+{maint.cust_phone_number}</td>
            <td>{maint.cust_created_date}</td>
            <td>{maint.cust_modified_date}</td>
            <td>{maint.cust_last_action}</td>
              <td>{maint.cust_active ? "Yes" : "No"}</td>
            
              <td>
                <button
                  onClick={() => navigate(`/inspect-customer/${maint.customer_id}`)}
